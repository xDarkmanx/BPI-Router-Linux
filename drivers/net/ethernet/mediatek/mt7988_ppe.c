...
static int mtk_ppe_wait_busy(struct mtk_ppe *ppe)
{
	...
	ret = readl_poll_timeout(ppe->base + MTK_PPE_GLO_CFG, val,
				 !(val & MTK_PPE_GLO_CFG_BUSY), 20,
				 MTK_PPE_WAIT_TIMEOUT_US);
...
static int mtk_mib_entry_read(struct mtk_ppe *ppe, u16 index, u64 *bytes,
			      u64 *packets)
{
	...
	/* 64 bit for each counter (NetSys V3) */
...
static inline u32 *mtk_foe_entry_ib2(struct mtk_eth *eth,
...
int mtk_foe_entry_set_pse_port(struct mtk_eth *eth, struct mtk_foe_entry *entry,
			       u8 port)
{
...
int mtk_foe_entry_set_ipv6_tuple(struct mtk_eth *eth,
				 struct mtk_foe_entry *entry, __be32 *src_addr,
			   ...
			   bool amsdu_en)
{
	...
	*ib2 |= FIELD_PREP(MTK_FOE_IB2_RX_IDX, txq) | MTK_FOE_IB2_WDMA_WINFO_V2;
...
static bool mtk_flow_entry_match(struct mtk_eth *eth,
...
static void __mtk_foe_entry_clear(struct mtk_ppe *ppe,
				  struct mtk_flow_entry *entry)
{
	...
	if (entry->hash != 0xffff) {
		...
		hwe->ib1 |=
			FIELD_PREP(MTK_FOE_IB1_STATE, MTK_FOE_STATE_INVALID);
		dma_wmb();
		mtk_ppe_cache_clear(ppe);
...
static void mtk_flow_entry_update_l2(struct mtk_ppe *ppe,
				     struct mtk_flow_entry *entry)
{
	u32 ib1_ts_mask = mtk_get_ib1_ts_mask(ppe->eth);
...
static void mtk_flow_entry_update(struct mtk_ppe *ppe,
				  struct mtk_flow_entry *entry)
{
...
static void __mtk_foe_entry_commit(struct mtk_ppe *ppe,
				   struct mtk_foe_entry *entry, u16 hash)
{
	...
	entry->ib1 |= FIELD_PREP(MTK_FOE_IB1_BIND_TIMESTAMP_V2, timestamp);

	hwe = mtk_foe_get_entry(ppe, hash);
	memcpy(&hwe->data, &entry->data,
	       eth->soc->foe_entry_size - sizeof(hwe->ib1));
	wmb();
	hwe->ib1 = entry->ib1;
...
static int mtk_foe_entry_commit_l2(struct mtk_ppe *ppe,
				   struct mtk_flow_entry *entry)
{
	...
	prev = rhashtable_lookup_get_insert_fast(
		&ppe->l2_flows, &entry->l2_node, mtk_flow_l2_ht_params);
	if (likely(!prev))
		return 0;
...
static void mtk_foe_entry_commit_subflow(struct mtk_ppe *ppe,
					 struct mtk_flow_entry *entry, u16 hash)
{
	const struct mtk_soc_data *soc = ppe->eth->soc;
...
mtk_foe_entry_get_mib(struct mtk_ppe *ppe, u32 index,
...
struct mtk_ppe *mtk_ppe_init(struct mtk_eth *eth, void __iomem *base, int index)
{
	...
	foe_flow_size =
		(MTK_PPE_ENTRIES / soc->hash_offset) * sizeof(*ppe->foe_flow);
	ppe->foe_flow = devm_kzalloc(dev, foe_flow_size, GFP_KERNEL);
	if (!ppe->foe_flow)
		goto err_free_l2_flows;

	if (accounting) {
		mib = dmam_alloc_coherent(ppe->dev,
					  MTK_PPE_ENTRIES * sizeof(*mib),
...
void mtk_ppe_start(struct mtk_ppe *ppe)
{
	...
	val = MTK_PPE_TB_CFG_AGE_NON_L4 | MTK_PPE_TB_CFG_AGE_UNBIND |
	      MTK_PPE_TB_CFG_AGE_TCP | MTK_PPE_TB_CFG_AGE_UDP |
	      MTK_PPE_TB_CFG_AGE_TCP_FIN |
	      FIELD_PREP(MTK_PPE_TB_CFG_SEARCH_MISS,
			 MTK_PPE_SEARCH_MISS_ACTION_FORWARD_BUILD) |
	      FIELD_PREP(MTK_PPE_TB_CFG_KEEPALIVE, MTK_PPE_KEEPALIVE_DISABLE) |
	      FIELD_PREP(MTK_PPE_TB_CFG_HASH_MODE, 1) |
	      FIELD_PREP(MTK_PPE_TB_CFG_SCAN_MODE,
			 MTK_PPE_SCAN_MODE_CHECK_AGE) |
	      FIELD_PREP(MTK_PPE_TB_CFG_ENTRY_NUM, MTK_PPE_ENTRIES_SHIFT);
	/* NetSys V3 specific flags */
	...
	val = MTK_PPE_FLOW_CFG_IP6_3T_ROUTE | MTK_PPE_FLOW_CFG_IP6_5T_ROUTE |
	      MTK_PPE_FLOW_CFG_IP6_6RD | MTK_PPE_FLOW_CFG_IP4_NAT |
	      MTK_PPE_FLOW_CFG_IP4_NAPT | MTK_PPE_FLOW_CFG_IP4_DSLITE |
	      MTK_PPE_FLOW_CFG_IP4_NAT_FRAG | MTK_PPE_MD_TOAP_BYP_CRSN0 |
	      MTK_PPE_MD_TOAP_BYP_CRSN1 | MTK_PPE_MD_TOAP_BYP_CRSN2 |
	...
	/* enable PPE */
	val = MTK_PPE_GLO_CFG_EN | MTK_PPE_GLO_CFG_IP4_L4_CS_DROP |
	      MTK_PPE_GLO_CFG_IP4_CS_DROP | MTK_PPE_GLO_CFG_FLOW_DROP_UPDATE |
...
int mtk_ppe_stop(struct mtk_ppe *ppe)
{
	...
	val = MTK_PPE_TB_CFG_AGE_NON_L4 | MTK_PPE_TB_CFG_AGE_UNBIND |
	      MTK_PPE_TB_CFG_AGE_TCP | MTK_PPE_TB_CFG_AGE_UDP |
	      MTK_PPE_TB_CFG_AGE_TCP_FIN | MTK_PPE_TB_CFG_SCAN_MODE;
